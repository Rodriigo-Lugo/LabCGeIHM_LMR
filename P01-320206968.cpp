...
#include <glew.h>
#include <glfw3.h>
#include <windows.h> //para ocuoar la funcion sleep
#include <random> // para ocupar la funcion de random
...
std::mt19937 generador(rd()); // Mersenne Twister
std::uniform_real_distribution<float> distribucion(0.0f, 1.0f); //flotantes entre 0.0 y 1.0
...
int main()
{
	...
	while (!glfwWindowShouldClose(mainWindow))
	{
		...
		//esperar que pasen 2 segundos para que cambie el color 
		double tiempoActual = glfwGetTime();
		if (tiempoActual - ultimoTiempo >= 2.0) {
			ultimoTiempo = tiempoActual;
			glClearColor(distribucion(generador), distribucion(generador), distribucion(generador), 1.0f);
		}
