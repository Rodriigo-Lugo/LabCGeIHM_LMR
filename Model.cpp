...
{
	Assimp::Importer importer;//					Pasa de Polygons y Quads a triangulos, modifica orden para el origen, generar normales si el  objeto no tiene, trata vrtices iguales como 1 solo
...
void Model::LoadMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	for (unsigned int i = 0; i < mesh->mNumVertices; i++)
	{
		...
		//Normals importante, las normales son negativas porque la luz interacta con ellas de esa forma, cmo se vio con el 
	...
	for (unsigned int i = 0; i < scene->mNumMaterials; i++)
	{
		...
		if (material->GetTextureCount(aiTextureType_DIFFUSE))
		{
			...
			if (material->GetTexture(aiTextureType_DIFFUSE, 0, &path) == AI_SUCCESS)
			{
				...
				{
					//printf("entre a 1 / \n");
					idx = std::string(path.data).rfind("/");//para quitar del path del modelo todo lo que este antes del \ de ubicacin de directorio
