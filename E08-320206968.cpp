...
int main()
{
	...
	pointLights[1] = PointLight(
		1.0f, 1.0f, 1.0f,   // color blanco
		1.0f, 1.5f,        // IA , ID 
		100.0f, 7.0f, 0.0f,  // posicion
		0.2f, 0.004f, 0.003f // atenuación
	);
	pointLightCount++;*/
	...
	//luz prueba
	spotLights[3] = SpotLight(1.0f, 1.0f, 1.0f,   //amarillo
		1.0f, 1.0f,         // IA, ID
		50.0f, 7.0f, 0.0f,   // pos
		0.0f, -1.0f, 0.0f,  // dir 
		1.0f, 0.0, 0.0,		// atenuacion
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, 4.5f, 30.0f));
		...
		modelPokeLampara = glm::translate(modelPokeLampara, glm::vec3(0.0f, -1.0f, -10.0f));
		...
		// JERARQUIA
		glm::vec3 posLuzPokeLocal = glm::vec3(0.0f, 0.8f, 0.0f);
		glm::vec3 posLuzPokeGlobal = glm::vec3(modelPokeLampara * glm::vec4(posLuzPokeLocal, 1.0f));
		// Actualiza la posición (jerarquía)
		pointLights[0].SetPos(posLuzPokeGlobal);//SetPos agregado en PointLight.h

		//antes de subir luces al shader calcula cuantas point lights enviar (0 si está apagada, 1 si encendida)
		pointCountAEnviar = mainWindow.getLuzPokeEncendida() ? 1 : 0;
		// Sube las luces (usa el "count" calculado)
		shaderList[0].SetPointLights(pointLights, pointCountAEnviar);

		//CONTENIDO DE LA PRACTICA 7
		/*
		//Instancia del coche 
		model = glm::mat4(1.0);
		...
		// LUZ FARO AZUL
		glm::vec3 DirFaroLocal = glm::vec3(1.0f, 0.0f, 0.0f); // dirección local
		glm::vec3 PosFaroLocal = glm::vec3(3.0f, 0.0f, 0.5f); // posicion local 

		glm::vec3 PosFaroGlobal = glm::vec3(modelauxLuz * glm::vec4(PosFaroLocal, 1.0f)); //posición global
		glm::vec3 DirFaroGlobal = glm::normalize(glm::vec3(modelauxLuz * glm::vec4(DirFaroLocal, 0.0f))); //dirección global

		spotLights[1].SetFlash(PosFaroGlobal, DirFaroGlobal);
		// ======================================================================

	
		//============HELICOPTERO==================
		modelHelicoptero = glm::mat4(1.0);
		modelHelicoptero = glm::translate(modelHelicoptero, glm::vec3(0.0f + mainWindow.getmueveHeli(), 5.0f, 6.0));
		modelHelicoptero = glm::scale(modelHelicoptero, glm::vec3(0.3f, 0.3f, 0.3f));
		modelHelicoptero = glm::rotate(modelHelicoptero, -90 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		modelHelicoptero = glm::rotate(modelHelicoptero, 90 * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
		modelauxLuz = modelHelicoptero;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(modelHelicoptero));
		Blackhawk_M.RenderModel();

		// LUZ Helicóptero
		glm::vec3 DirLuzHeliLocal = glm::vec3(0.0f, 0.0f, -1.0f); // dirección local
		glm::vec3 PosLuzHeliLocal = glm::vec3(0.0f, 0.0f, -1.0f); // posicion local 

		glm::vec3 PosLuzHeliGlobal = glm::vec3(modelauxLuz * glm::vec4(PosLuzHeliLocal, 1.0f)); //posición global
		glm::vec3 DirLuzHeliGlobal = glm::normalize(glm::vec3(modelauxLuz * glm::vec4(DirLuzHeliLocal, 0.0f))); //dirección global

		spotLights[2].SetFlash(PosLuzHeliGlobal, DirLuzHeliGlobal);
		// ======================================================================*/
